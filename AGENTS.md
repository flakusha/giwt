...
1. **Settings** (`src/utils/settings.ts`): layered TOML — defaults < `~/.config/giwt/config.toml` < `<repo>/giwt.toml` < env. Schema v1 (`[branches]`, `[paths]`, `[commands]`, `[runlog]`, `[output]` — `format` + `stream_tail`, the console tail cap for failing check/test streams, default 25); unknown keys warn + are ignored; wrong types hard-error.
